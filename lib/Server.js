...
const PiVPNOpenVpn = require('./PiVPNOpenVpn');
  ...
  constructor() {
    ...
    this.app = express()
      ...
      .post('/api/session', Util.promisify(async req => {
        ...
        const openvpn = new PiVPNOpenVpn({ ssh });
      ...
      // OpenVPN
      .get('/api/openvpn/client', Util.promisify(async req => {
        const { openvpn } = this.getSession(req.session.id);
        return openvpn.getClients();
      }))
      .get('/api/openvpn/client-status', Util.promisify(async req => {
        const { openvpn } = this.getSession(req.session.id);
        return openvpn.getClientsStatus();
      }))
      .get('/api/openvpn/client/:name', Util.promisify(async req => {
        const { name } = req.params;
        const { openvpn } = this.getSession(req.session.id);
        return openvpn.getClient({ name });
      }))
      .get('/api/openvpn/client/:name/qrcode.svg', Util.promisify(async (req, res) => {
        const { name } = req.params;
        const { openvpn } = this.getSession(req.session.id);
        const svg = await openvpn.getClientQRCodeSVG({ name });
        res.header('Content-Type', 'image/svg+xml');
        res.send(svg);
      }))
      .get('/api/openvpn/client/:name/configuration', Util.promisify(async (req, res) => {
        const { name } = req.params;
        const { openvpn } = this.getSession(req.session.id);
        const configuration = await openvpn.getClientConfiguration({ name });
        res.header('Content-Disposition', `attachment; filename="${name}.ovpn"`);
        res.header('Content-Type', 'text/plain');
        res.send(configuration);
      }))
      .post('/api/openvpn/client/:name', Util.promisify(async req => {
        const { name } = req.params;
        const { openvpn } = this.getSession(req.session.id);
        return openvpn.createClient({ name });
      }))
      .delete('/api/openvpn/client/:name', Util.promisify(async req => {
        const { name } = req.params;
        const { openvpn } = this.getSession(req.session.id);
        return openvpn.deleteClient({ name });
      }))
      .post('/api/openvpn/client/:name/enable', Util.promisify(async req => {
        const { name } = req.params;
        const { openvpn } = this.getSession(req.session.id);
        return openvpn.enableClient({ name });
      }))
      .post('/api/openvpn/client/:name/disable', Util.promisify(async req => {
        const { name } = req.params;
        const { openvpn } = this.getSession(req.session.id);
        return openvpn.disableClient({ name });
      }))
